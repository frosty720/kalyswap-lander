	...
	return (
		<section id="app" className="relative w-full py-24">
			<div className="mx-auto max-w-[1240px] px-6">
				...
				<Reveal className="rounded-3xl border hairline bg-surface/50 p-4 sm:p-6 shadow-[0_40px_120px_-40px_rgba(245,158,11,0.25)]">
					<div className="flex items-center justify-between px-2 pb-4">
						...
						<span className="inline-flex items-center gap-2 text-xs font-display text-cream/50">
							<span className="w-2 h-2 rounded-full bg-emerald-400" /> {t.chainLabel}
						</span>
					</div>
					...
					<div className="grid lg:grid-cols-3 gap-4">
						...
						<div className="rounded-2xl border hairline bg-ink/60 p-6">
							<span className="text-xs font-display tracking-widest text-cream/40 uppercase">{t.liveStats}</span>
							<div className="mt-4 space-y-4 text-sm font-display">
								{STAT_KEYS.map((key) => (
									<div key={key} className="flex items-center justify-between border-b hairline pb-3 last:border-0">
										<span className="text-cream/55">{statLabels[key]}</span>
					...
					<div className="grid lg:grid-cols-3 gap-4 mt-4">
						<div className="lg:col-span-2 rounded-2xl border hairline bg-ink/60 p-6">
							<div className="flex items-center justify-between mb-4">
								<span className="font-display font-bold text-lg">{t.myAssets}</span>
								<span className="text-xs font-display text-gold">{t.viewAll}</span>
							</div>
								...
								<table className="w-full text-sm font-display">
									<thead>
										<tr className="text-left text-[11px] tracking-widest text-cream/40 uppercase">
											...
											<th className="pb-3 font-medium text-right">{t.colBalance}</th>
											<th className="pb-3 font-medium text-right">{t.colPrice}</th>
											<th className="pb-3 font-medium text-right">{t.colValue}</th>
											<th className="pb-3 font-medium text-right">{t.col24h}</th>
										</tr>
									</thead>
									<tbody>
										{ASSETS.map((asset) => (
						...
						<div className="rounded-2xl border hairline bg-ink/60 p-6">
							<div className="flex items-center justify-between mb-4">
								<span className="font-display font-bold text-lg">{t.activeYields}</span>
							</div>
				...
				<p className="text-center text-xs text-cream/35 mt-5 font-display">{t.disclaimer}</p>
			</div>
		</section>
	);
}
